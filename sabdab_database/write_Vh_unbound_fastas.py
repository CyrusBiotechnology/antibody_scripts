...
print '\nmaking Fh fasta files for %i unbound Fab structures'%(len(d))
print infile.rsplit('_')[1][:-4]
#fout = open('unbound_%s/%s_Fh'%(infile.rsplit('_')[:-4]),'w')
for key in d:
#  print key
  fasta_list = []
  # get fasta sequences for specified heavy chains
  ...
  if len(d[key]) > 1:
    if len(set(fasta_list)) > 1:
      ...
      if len(set(fasta_list)) != len(d[key]):
        sys.exit('number of unique heavy chain sequences not equal to the number of total heavy chains in %s'%(key))
      else:
        for x in range(len(fasta_list)):
          fname = 'unbound_%s/heavy/%s_%s_Fh.fasta'%(infile.rsplit('_')[1][:-4],key,d[key][x])
    ...
    fname = 'unbound_%s/heavy/%s_%s_Fh.fasta'%(infile.rsplit('_')[1][:-4],key,d[key][0])
    if not os.path.exists(os.path.dirname(fname)):
      os.makedirs(os.path.dirname(fname))
    fout = open(fname, 'w')
    fout.write('>%s_%s\n'%(key,d[key][0]))
